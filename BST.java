...
public class BST<T> {
    ...
    public Photo search(String k) {
    	if(findKey(k)) {
        BSTNode<T> p = root;
        while (p != null) {
            current = p;
            if (k.compareTo(p.key) == 0) {
                return (Photo) current.data;
            } else if (k.compareTo(p.key) < 0) {
                p = p.left;
            } else {
                p = p.right;
    ...
    public int numOfComparation(String k) {
	...
	public int numberOfComparisons(String condition) {
		int nb = 0;
		BSTNode<T> current = root;
		while (current != null) {
			nb++;
			if(condition.compareTo(current.key) < 0) 
				current = current.left;
			else if(condition.compareTo(current.key) == 0)  
				return nb;
			else 
